---
title: "Exam Score Prediction For Public School Students"
author: "Chhiring Lama, Flora Zang, Jeong Yun Choi, Kate Colvin"
mainfont: Arial
fontsize: 11pt
output: 
  pdf_document:
...
\vspace{-0.4cm}
  The full dataset includes data from 948 students. We created summary tables to describe the distributions of all variables, stratified by gender ([Table 1-3](#tab1_2_3-section)). To further investigate the distribution of test scores by subject, we created visualizations including stratified density and boxplots, and a correlation matrix of the test scores ([Fig. 1-3](#fig1_2_3-section)). Most variables were missing between 15 to 100 values, spread evenly across the students. Students with missing data were included in all exploratory analysis and visualizations, and were excluded from regression models when necessary. We found a handful of score outliers for each subject, but chose to leave these points in, as they seemed like valid data points (and not errors in data entry). Other minor data cleaning was performed. 

\vspace{-0.4cm}
## Model Development, Diagnostics and Selection
\vspace{-0.4cm}
  We analyzed covariates with high correlation, we performed a series of chi-square tests and one-way ANOVA to determine whether the covariates were dependent ([Table 4-5](#tab4_5-section)). These tests helped us identify pairs of variables with the weakest associations, allowing us to select five variables that were most independent of each other. These variables formed the basis of our initial linear model for stepwise-regression model selection. The missing data were dropped because imputed values for categorical variables may complicate the interpretability of the result and it adds uncertainty that needs to be accounted for.

\vspace{-0.4cm}
### Math Score Prediction
\vspace{-0.4cm}
  With forward selection, we obtained seven significant predictors. Subsequently,we visualized math score between pairs of categorical variables. Among the pairs where the mean values score are associated within sub-categories, we tested for interaction, and found that weekly study hours and parental marital status has significant combined effect ([Fig. 4](#fig4-section)). Upon examining diagnostic plots for the selected model, the residual plot revealed greater variance for lower values as well as deviation from normality in Q-Q plot ([Fig. 5, Table 6](#fig5-section)). To address this, we applied a transformation of $(Math\ Score + 1)^{1.3}$, as determined by box-cox, to the math scores variable, improving its compatibility with the model ([Fig. 6-7, Table 7-9](#fig6_7-section)). Step-wise regression also resulted in the same model. Based on the Mallow’s Cp and adjusted $R^2$, an optimal model for math score is suggested to have 15-17 main effect parameters. This aligns with our model selected from forward and backward model selection. LASSO suggests taking number of siblings into account ([Fig. 8, Table 10](#fig8_tab10-section)). So, we decided to cross-validate and compare performances between model with and without number of sibling as a modifier. 

\vspace{-0.4cm}
### Writing Score Prediction
\vspace{-0.4cm}
  With forward selection, we identified eight significant covariates to predict writing score, among which seven were the same as identified for model predicting math score. We applied similar steps to check interactions and uncovered significant interaction coefficient for `gender:wkly_study_hours` and `lunch_type:wkly_study_hours`([Fig. 9, Table 11-12](#fig9-section)). The same end model was selected upon step-wise regression selection. The residuals followed homoscedascticity, mean zero assumptions, but deviated from normality assumptions. Therefore, box-cox transformation was used to identify the optimal transformation, and model was re-fitted using $\sqrt(Writing\ Score)^3$([Fig. 10-12, Table 13](#fig10_12-section)). We also checked for outliers and influential points as a part of the diagnostics. The models were further investigated using test based criteria. Based on the Mallow’s Cp and adjusted $R^2$, an optimal model for writing score will should 13-18 main effect parameters ([Fig. 13](#fig13_tab13-section)). This aligns with our model selected from forward and step-wise model selection. Like in math score model, LASSO suggests taking number of siblings into account ([Table 14](#fig13_tab13-section)), so, we decided to cross-validate and compare performances between model with and without number of sibling as a modifier and the step-wise model to predict writing score as well. 

\vspace{-0.4cm}
### Reading Score Prediction
\vspace{-0.4cm}
  We implemented forward selection regression to deduce significant covariates for modeling `reading score`. We applied similar steps to check interactions and uncovered significant interaction coefficient for `gender:wkly_study_hours`. ([Fig. 14, Table 15-16](#fig14-section)). Similar result was selected upon using step-wise regression selection. We observed that the residuals followed the assumptions, except for normality. Therefore, box-cox transformation was used to identify the optimal transformation and the model was refitted. ([Fig. 15-17, Table 17](#fig15-17-section)). We again checked for outliers and influential points. The model as further deduced using Mallow's Cp and adjusted $R^2$ for further diagnostics.([Fig. 18](#fig18-section)). Finally, LASSO demonstrated taking number of siblings into account as did in `writing score`. ([Table 18](#fig18-section)). We again will conduct cross-validation to compare performances between model with and without number of siblings as a modifier to predict reading score.
...
\vspace{-0.4cm}
  ...
  All of the models without sibling counts have lower root mean square error and higher $R^2$ (excluding reading score) has similar predictive ability as its corresponding more complex model ([Table 19-21, Fig. 19-21](#fig1921-section)).\
...
```{r, echo=FALSE, warning = FALSE, message = FALSE, fig.width = 6, fig.height = 2, out.width = "95%", dpi = 600}
...
```{r, echo=FALSE, warning = FALSE, message = FALSE, fig.width = 3, fig.height = 3, out.width = "95%", dpi = 600}
reading_score_df <- score_df |> 
  select(-c(math_score, writing_score))
covars <- names(reading_score_df)[names(reading_score_df) != "reading_score"]

individual_fits <- lapply(covars, function(vars){
  fit1 = lm(`reading_score` ~ get(vars), data = reading_score_df) |> 
  broom::tidy() |> 
    mutate(term = str_replace(term, "get[(]vars[])]", vars))
})
...
# Enter first the one with the lowest p-value: `test_prep`
forward1 = lm(`reading_score` ~ `gender`, data = reading_score_df)
#summary(forward1)

### Step 2: Enter the one with the lowest p-value in the rest 
covars <- names(reading_score_df)[!names(reading_score_df) %in% c("reading_score_score", "gender")]
step2_fits <- lapply(covars, function(vars){
  fit1 = lm(`reading_score` ~ gender + get(vars), data = reading_score_df) |> 
  broom::tidy() |> 
    mutate(term = str_replace(term, "get[(]vars[])]", vars), 
           group = paste("forward1", vars, sep = "+"))
})
...
### Step 3: Enter the one with the lowest p-value in the rest 
covars <- names(reading_score_df)[!names(reading_score_df) %in% c("reading_score", "lunch_type", "gender")]
step3_fits <- lapply(covars, function(vars){
  fit1 = lm(`reading_score` ~  lunch_type + gender + get(vars), data = reading_score_df) |> 
  broom::tidy() |> 
    mutate(term = str_replace(term, "get[(]vars[])]", vars), 
           group = paste("forward2", vars, sep = "+"))
})
...
### Step 4: Enter the one with the lowest p-value in the rest 
covars <- names(reading_score_df)[!names(reading_score_df) %in% c("reading_score", "gender", "lunch_type", 
                                                                  "test_prep")]
step4_fits <- lapply(covars, function(vars){
  fit1 = lm(`reading_score` ~ gender + lunch_type + test_prep + get(vars), data = reading_score_df) |> 
  broom::tidy() |> 
    mutate(term = str_replace(term, "get[(]vars[])]", vars), 
           group = paste("forward3", vars, sep = "+"))
})
...
### Step 5: Enter the one with the lowest p-value in the rest 
covars <- names(reading_score_df)[!names(reading_score_df) %in% c("reading_score", "gender", "lunch_type", 
                                                                  "test_prep", "parent_educ")]
step5_fits <- lapply(covars, function(vars){
  fit1 = lm(`reading_score` ~ gender + lunch_type + test_prep + 
              parent_educ + get(vars), data = reading_score_df) |> 
  broom::tidy() |> 
    mutate(term = str_replace(term, "get[(]vars[])]", vars), 
           group = paste("forward4", vars, sep = "+"))
})
...
### Step 6: Enter the one with the lowest p-value in the rest 
covars <- names(reading_score_df)[!names(reading_score_df) %in% c("reading_score", "gender", "lunch_type", 
                                                                  "test_prep", "parent_educ", "ethnic_group")]
step6_fits <- lapply(covars, function(vars){
  fit1 = lm(`reading_score` ~ test_prep + gender + lunch_type + 
              parent_educ + ethnic_group + get(vars), 
            data = reading_score_df) |> 
  broom::tidy() |> 
    mutate(term = str_replace(term, "get[(]vars[])]", vars), 
           group = paste("forward5", vars, sep = "+"))
})
...
# Enter the one with the lowest p-value: `is_first_child`
forward6 = update(forward5, . ~ . + is_first_child)
#summary(forward6)

### Step 7: Enter the one with the lowest p-value in the rest 
covars <- names(reading_score_df)[!names(reading_score_df) %in% c("reading_score", "gender", "test_prep", 
                                                                  "lunch_type", "parent_educ", "ethnic_group", 
                                                            "is_first_child")]
step7_fits <- lapply(covars, function(vars){
  fit1 = lm(`reading_score` ~ test_prep + gender + lunch_type + 
              parent_educ + ethnic_group + is_first_child + get(vars), 
            data = reading_score_df) |> 
  broom::tidy() |> 
    mutate(term = str_replace(term, "get[(]vars[])]", vars), 
           group = paste("forward6", vars, sep = "+"))
...
# Enter the one with the lowest p-value: `parent_marital_status`
forward7 = update(forward6, . ~ . + parent_marital_status)
#summary(forward7)
### Step 7: Enter the one with the lowest p-value in the rest 
covars <- names(reading_score_df)[!names(reading_score_df) %in% c("reading_score", "gender", "test_prep", 
                                                                  "lunch_type", "parent_educ", "ethnic_group", 
                                                            "is_first_child", "parent_marital_status")]
step8_fits <- lapply(covars, function(vars){
  fit1 = lm(`reading_score` ~ test_prep + gender + lunch_type + 
              parent_educ + ethnic_group + is_first_child + parent_marital_status + get(vars), 
            data = reading_score_df) |> 
  broom::tidy() |> 
    mutate(term = str_replace(term, "get[(]vars[])]", vars), 
           group = paste("forward7", vars, sep = "+"))
...
step8_fits <- Reduce(rbind, step8_fits) |> 
  filter(term != "(Intercept)") 
#Newly added variable p value is greater 0.05 so we stop here
```

## {#fig14-section}
```{r, echo=FALSE, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 4, out.width = "95%", dpi = 600}
covars <- c("gender", "test_prep","lunch_type", "parent_educ", "ethnic_group", "is_first_child", "parent_marital_status", "wkly_study_hours")

list1 <- list()
for (index in 1:(length(covars)-1)) {
  for(i in (index+1): length(covars)) {
    subset_df <- reading_score_df |> 
      drop_na(rlang::sym(covars[index]), rlang::sym(covars[i]))
    p <- ggline(subset_df, # remove NA level for color group
                   x = covars[index], 
                   y = "reading_score", color = covars[i],
                   add = c("mean")) +
      theme(text = element_text(size = 5), 
            legend.position = "right", 
            panel.spacing.x=unit(0, "lines"),
...
commonplot <- ggarrange(list1$`gender:wkly_study_hours`, ncol = 1, 
          legend = "right", 
          label.y = "Reading Score") 

annotate_figure(commonplot, top = "Figure 14: Interaction Between Covariates for Reading Score (Significant in Prediction)")
##potential association between covariates in relation to their math score (adding significant interactions only)
forward8 = update(forward7, . ~ . + gender*wkly_study_hours) 
#summary(forward8)
...
```{r, echo = FALSE}
## transform reading score 
reading_score_df <- reading_score_df |> 
  mutate(transformed_reading = (reading_score)^1.39) 

mult.forward.final <- lm(formula =transformed_reading ~ gender + test_prep + lunch_type + 
    parent_educ + ethnic_group + is_first_child + parent_marital_status + 
    wkly_study_hours + gender:wkly_study_hours, data = reading_score_df)
...
reading_model2v1 <- lm(transformed_reading ~ gender + test_prep + parent_marital_status + wkly_study_hours + ethnic_group * parent_educ, data = reading_score_df)
...
reading_model2v2 <- lm(transformed_reading ~ gender + parent_educ + lunch_type + ethnic_group + test_prep + parent_marital_status + wkly_study_hours + parent_marital_status*wkly_study_hours, data = reading_score_df)
...
reading_model2v3 <- lm(transformed_reading ~ gender + lunch_type + parent_marital_status + gender*parent_educ + test_prep*parent_educ, data = reading_score_df)
...
par(mfrow = c(2,2))
plot(mult.forward.final)
mtext("Figure 15: Reading Score Model from Forward Selection", side = 3, line = - 2, outer = TRUE)

mult.forward.final <- lm(formula =reading_score ~ gender + test_prep + lunch_type + 
    parent_educ + ethnic_group + is_first_child + parent_marital_status + 
    wkly_study_hours + gender:wkly_study_hours, data = reading_score_df)
...
boxcox <- MASS::boxcox(mult.forward.final, 
             lambda = seq(-2.5, 2.5, 1/10)) 
mtext("Figure 16: Box-cox Likelihood", side = 3, line = - 2, outer = TRUE)
...
par(mfrow = c(2,2))
plot(mult.forward.final)
mtext("Figure 17: Assumptions for Reading Score Model After Transformation", side = 3, line = - 2, outer = TRUE)
```

## {#fig18-section}
```{r, echo=FALSE, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 4, out.width = "95%", dpi = 600}
unselect_reading_score_df <- reading_score_df |> 
  select(-reading_score)

mat = as.matrix(reading_score_df)
# Printing the 2 best models of each size, using the Cp criterion:
#leaps(x = mat[,c(1:2, 4:8)], y = mat[,3], nbest = 2, method = "Cp")
...
# Function regsubsets() performs a subset selection by identifying the "best" model that contains
# a certain number of predictors. By default "best" is chosen using SSE/RSS (smaller is better)
b = regsubsets(`transformed_reading` ~ . , data = unselect_reading_score_df, 
               nvmax = 30)
rs = summary(b)

reading_score_subdf <- reading_score_df |> 
  drop_na()

# fit a LASSO with lambda = 5
fit_5 <- glmnet(as.matrix(reading_score_subdf[1:11]), reading_score_subdf$transformed_reading, lambda = 5)
#coef(fit_5)

# fit a LASSO with lambda = 1
fit_1 <- glmnet(as.matrix(reading_score_subdf[1:11]), reading_score_subdf$transformed_reading, lambda = 1)
#coef(fit_1)

# fit a LASSO with lambda = 0.1
fit_0.1 <- glmnet(as.matrix(reading_score_subdf[1:11]), reading_score_subdf$transformed_reading, lambda = 0.1)
#coef(fit_0.1)

# using cross validation to choose lambda
lambda_seq <- 10^seq(-3, 0, by = .1)
set.seed(2)
cv_object <- cv.glmnet(as.matrix(reading_score_subdf[1:11]), reading_score_subdf$transformed_reading, 
                       lambda = lambda_seq, 
                       nfolds = 5)
#cv_object 
...
# ftiiting the lasso model with the "best" lambda
fit_bestcv <- glmnet(as.matrix(reading_score_subdf[1:11]), reading_score_subdf$transformed_reading, lambda = cv_object$lambda.min) 
#coef(fit_bestcv)
...
plot(2:23, rs$adjr2, xlab="No of parameters", ylab="Adj R2")
mtext("Figure 18: Test-based Procedures For Reading Score", side = 3, line = - 2, outer = TRUE, cex = 1.5)

fit_bestcv |> 
  broom::tidy() |> 
  mutate_if(is.numeric, round, 3) |> 
  knitr::kable(caption = "LASSO Model For Reading Score")
...
filtered_df |> 
  ...
  labs(x = "Predicted Score", y = "Actual Score", 
       title = "Figure 19: Correlation of Observed and Predicted Math Score") +
...
filtered_df |> 
  ...
  geom_point(size = 1, shape = 21) +
  labs(x = "Predicted Score", y = "Actual Score", 
       title = "Figure 20: Correlation of Observed and Predicted Writing Score") +
  facet_wrap(~model_type, scale = "free") +
  stat_cor(label.y = 5) +
...
filtered_df <- reading_score_df |> 
  drop_na(reading_score, transformed_reading, gender, test_prep, 
    lunch_type, parent_educ, ethnic_group, is_first_child, parent_marital_status, 
    wkly_study_hours, nr_siblings)

# Fit 1st model that we discussed in forward model without number of siblings
model1 = train(transformed_reading ~ gender + test_prep + lunch_type + 
    parent_educ + ethnic_group + is_first_child + parent_marital_status + 
    wkly_study_hours + gender:wkly_study_hours ,
    data = filtered_df,
    trControl = train,
...
# Fit 2nd model that we discussed in forward model with number of siblings
model2 = train(transformed_reading ~ gender + test_prep + lunch_type + 
    parent_educ + ethnic_group + is_first_child + parent_marital_status + 
    wkly_study_hours + gender:wkly_study_hours + nr_siblings, 
    data = filtered_df,
    trControl = train,
    method = 'lm',
    na.action = na.pass)
...
```{r, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 9, fig.height = 3, out.width = "95%", dpi = 600}
# get fold subsets
fold_data_model1 <- lapply(model1$control$index, function(index) reading_score_df[index,]) |> 
    bind_rows(.id = "Fold") |> 
  mutate(reading_score = transformed_reading^(1/1.39))
fold_data_model2 <- lapply(model2$control$index, function(index) reading_score_df[index,]) |> 
    bind_rows(.id = "Fold") |> 
  mutate(reading_score = transformed_reading^(1/1.39))


# example plots
plot1 <- ggplot(fold_data_model1, aes(reading_score, col = Fold)) + geom_density(alpha = 0.6) + ggtitle("Model w/o Number of Siblings")
plot2 <- ggplot(fold_data_model2, aes(reading_score, col = Fold)) + geom_density(alpha = 0.6) + ggtitle("Model w/o Number of Siblings")
#ggarrange(plot1, plot2, ncol = 2, common.legend = TRUE, 
        #  legend = "right")
...
```{r, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 8, fig.height = 3, out.width = "95%", dpi = 600}
filtered_df |> 
  mutate(mod_wo_sibling_count = (model1[["finalModel"]][["fitted.values"]])^(1/1.39), 
         mod_w_sibling_count = (model2[["finalModel"]][["fitted.values"]])^(1/1.39))|> 
  pivot_longer(
    cols = c(mod_wo_sibling_count, mod_w_sibling_count), 
    names_to = "model_type", 
    values_to = "res"
  ...
  geom_point(size = 1, shape = 21) +
  labs(x = "Predicted Score", y = "Actual Score", 
       title = "Figure 21: Correlation of Observed and Predicted Reading Score") +
  facet_wrap(~model_type, scale = "free") +
  stat_cor(label.y = 5) +
...
bind_rows(model1[["results"]], 
  ...
  mutate(model_id = c("w/o no. of sibling", "w no. of sibling")) |> 
  select(c(model_id, `RMSE`:`MAESD`)) |> 
  knitr::kable(digits =3, caption = "Performance matrices of the 2 Models in Predicting (Reading Score) ^1.39")
```
