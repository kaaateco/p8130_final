...
## significant interaction effect for gender, test_prep, lunch_type, parent_educ
```
```{r}
reading_model2 <- lm(transformed_reading ~ gender + test_prep + ethnic_group + lunch_type , data = score_df)
...
```{r}
### do gender and wkly study hour/test_prep/ethnic_group have combined effect?
reading_model3v1 <- lm(transformed_reading ~ gender*parent_educ + wkly_study_hours*parent_educ + test_prep*parent_educ + ethnic_group*parent_educ, data = score_df)
summary(reading_model3v1)

### no significant interaction effect and adjusted R^2 decreasing so, not choosing these interactions

### how about interaction between wkly study hour/parent marital status/test prep/ethnic group/lunch type
reading_model3v2 <- lm(transformed_reading ~ gender + wkly_study_hours*parent_marital_status + test_prep*parent_marital_status + ethnic_group*parent_marital_status + lunch_type*parent_marital_status + parent_educ*parent_marital_status, data = score_df)
summary(reading_model3v2)

### adjusted R^2 slightly improved, only keeping significant interactions

## how about only including some interaction between wkly_study_hr/ parent marital status/ parent educ
reading_model3v3 <- lm(transformed_reading ~ gender + wkly_study_hours*parent_marital_status + test_prep + ethnic_group + lunch_type + parent_educ*parent_marital_status, data = score_df)
...
```{r}
reading_model3 <- lm(transformed_math ~ gender + wkly_study_hours*parent_marital_status + test_prep + ethnic_group + lunch_type + parent_educ*parent_marital_status, data = score_df)
